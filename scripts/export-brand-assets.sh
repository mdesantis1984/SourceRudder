...
"${image[@]}" "$master" -strip -colorspace sRGB -filter Lanczos -resize 1280x640! \
  -define png:compression-level=9 "$export_root/sourcerudder-social-preview.png"
"${image[@]}" "$master" -strip -colorspace sRGB -filter Lanczos -resize 1200x600! \
  -background '#071521' -gravity center -extent 1200x630 -define png:compression-level=9 \
  "$export_brand/sourcerudder-og-1200x630.png"
"${image[@]}" "$master" -strip -colorspace sRGB -filter Lanczos -resize 1200x600! \
  -background '#071521' -gravity center -extent 1200x627 -define png:compression-level=9 \
  "$export_brand/sourcerudder-linkedin-1200x627.png"
"${image[@]}" "$master" -strip -colorspace sRGB -filter Lanczos -resize 1600x800! \
  -define png:compression-level=9 "$export_brand/sourcerudder-x-1600x800.png"

"${image[@]}" "$master" -strip -colorspace sRGB -crop 760x760+0+64 +repage \
